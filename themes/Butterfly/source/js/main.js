$(function() {
  ...
  //自定义阅读统计  span#busuanzi_value_page_pv
  function leancloudSelector(url) {
    url = encodeURI(url);
    return document.getElementById("leancloud-visitors-count");
  }
  function addCount(Counter) {
    var visitors = document.querySelector(".leancloud_visitors");
    var title = document.title.split("|")[0].trim(); //decodeURI(visitors.getAttribute('id'));
    var url = location.href.split(location.host)[1]; //visitors.getAttribute('data-flag-title');
      ...
      .then(({ results }) => {
        if (results.length > 0) {
          ...
          Counter("put", "/classes/Counter/" + counter.objectId, {
            time: { __op: "Increment", amount: 1.5 }
          }).then(response => response.json());
        } else {
  ...
  function showTime(Counter) {
    ...
    Counter(
      ...
      .then(({ results }) => {
        if (results.length === 0) {
          document
            .querySelectorAll(".leancloud_visitors .leancloud-visitors-count")
            .forEach(element => {
              element.innerText = 0;
            });
          return;
  ...
  fetch(
    "https://app-router.leancloud.cn/2/route?appId=9fgm6JVbw8PrOlvp7LfsKjsh-gzGzoHsz"
  )
    .then(response => response.json())
    .then(res => {
      var api_server = res.api_server; //.split('{')[2].split(' ')[1]
      var Counter = (method, url, data) => {
        return fetch(`https://` + api_server + `/1.1${url}`, {
          method: method,
          headers: {
            "X-LC-Id": "9fgm6JVbw8PrOlvp7LfsKjsh-gzGzoHsz",
            "X-LC-Key": "LReLvzSwPnBOAz5LiIgJsiDO",
